...
sub etl {
        ...
        csz_id => $self->_schema->resultset('CityStateZip')->find(
            {
                ...
                state_abbr => trim($self->legacy_friend->city_state_zip->state),
                zip => trim($self->legacy_friend->city_state_zip->postal_code),
            },
    ...
    die 'Address for Friend ' . $self->legacy_friend->friend_id
    ...
    die sprintf('Friend %d has no physical address!', $self->legacy_friend->id)
...
sub _process_address {
    ...
    if (
        ...
        push @addresses, {
            map { $_ => $field{$_} } grep { $field{$_} } keys %field
        };
    }
